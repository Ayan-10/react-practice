...
export default function WeatherPanel({ city }) {
  ...
  useEffect(() => {
    ...
    // TODO (fix): `city` is missing from the dependency array below, so this
    //             effect only runs once (on mount). When the user selects a
    //             different city, the effect does NOT re-run and the displayed
    //             weather stays stale.
    //
    //             Add `city` to the dependency array: `}, [city]);` so the
    //             weather refetches whenever the selected city changes.
  }, [city]);
